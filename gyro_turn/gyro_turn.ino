...
static int next_yaw_idx = 0;
const int n_yaw_sum = 100;
static int yaws[n_yaw_sum];
static long yaw_sum = 0;
...
void loop()
{
  ...
  const unsigned long target_z = 3550000L;
  
  while((millis() - last_off_target) < 100L)
  {
  //  z_total += yaw;
    z_total += long(get_yaw());

     unsigned long amount_off_target = abs(z_total - target_z);
     if(amount_off_target > 75000L) {
       last_off_target = millis();
     }
     
  Serial.println(amount_off_target);
     
     int mtr_pwr = max(130, min(160, amount_off_target / 15000L));
...
/*
  next_yaw_idx = (next_yaw_idx + 1) % n_yaw_sum;
  yaw_sum -= yaws[next_yaw_idx];
  yaws[next_yaw_idx] = raw_yaw;
  yaw_sum += raw_yaw;
  
  const int yaw_avg = int(yaw_sum / n_yaw_sum);
  
  Serial.print(yaw_avg, DEC);
  Serial.println(F(""));

  long yaw = long(raw_yaw - yaw_avg);
*/
/*
  Serial.print(F(" "));
  Serial.print(yaw);
  Serial.print(F("\t"));
  Serial.print(z_total, DEC);
  Serial.println(F(""));
  */
