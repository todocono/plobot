...
void init_movement() {
  ...
  */
  ...
  // TCCR1B = (TCCR1A & 0b00000000) | 0x03;
  TCCR1B = (TCCR1B & 0b00111000) | 0x01;
  //  TCCR2B = (TCCR1A & 0b00000000) | 0x04;
...
void turn(int degs)
{
  ...
  PID myPID(&Input, &Output, &Setpoint, 11 / scale_factor, 6 / scale_factor, 4 / scale_factor, DIRECT); //Kp Ki  Kd

  myPID.SetMode(AUTOMATIC);
  // Max should be <255 for bootstrap
  myPID.SetOutputLimits(-65, 65);
  int min_pwr = MIN_POWER + 35;
...
void move_straight(int pulses)
{
  ...
  int min_pwr = MIN_POWER, max_pwr = MAX_POWER;   // Max should be <255 for bootstrap
  long last_not_moving = millis() + 100;          // Grace period to start moving
  ...
  while ((millis() - last_off_target) < 500 && (millis() - started_move) < 2000L) {
    ...
    const int triangle = (avg_pulses > pulses / 2) ? (pulses - 8 * avg_pulses) : 2 * avg_pulses;
