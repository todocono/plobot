...
#define ticks_turn 1690 // 1420  for big transparent laser cut wheels (fat)
...
int left_power, right_power;
      ...
      if (iclp < (pulses / 30)) {
        //adjusts the speed of the motors according to how many steps have passed
        left_power = max(150, 255 - iclp / 30);
      }
      else {
        //adjusts the speed of the motors according to how many steps are remaining
        left_power = min(255, 150 + abs(iclp - pulses) / 30);
      }

      if (icrp < (pulses / 30)) {
        right_power = max(150, 255 - icrp / 30);
      }
      else {
        right_power = min(255, 150 + abs(icrp - pulses) / 30);
      }


      //deactivates left motor if right motor is way ahead
      ...
      //deactivates right motor if left motor is way ahead
