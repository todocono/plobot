...
#define ticks_fwd 5000
#define ticks_turn 1690 // 1420  

const int mtr_r_speed = 240, mtr_l_speed = 240;

const boolean turn_dirs[] = {
  false,
  true, 
...
const int n_turns = sizeof(turn_dirs) / sizeof(*turn_dirs);
...
void loop() {
  ...
  const int pulses = dir ? ticks_turn : ticks_fwd;
  ...
  */
  ...
  while (true) {
    ...
    if(abs(iclp-pulses) < 32 && abs(icrp-pulses) < 32) {
      ...
      const int left_power = min(200, 150 + abs(iclp - pulses) / 30);
      const int right_power = min(200, 150 + abs(icrp - pulses) / 30);
      
      if(iclp < (pulses - 16) && iclp < (icrp + 8)) {
        //const int power = (iclp < (icrp + 8)) ? left_power : 150;
        if(dir) {
          analogWrite(motor_l_fwd, 0);
          analogWrite(motor_l_back, left_power);
        } else {
          analogWrite(motor_l_fwd, left_power);
      ...
      if(icrp < (pulses - 16) && icrp < (iclp + 8)) {
       // const int power = (icrp < (iclp + 8)) ? right_power : 150;
        digitalWrite(motor_r_back, LOW);
        digitalWrite(motor_r_fwd, HIGH);
